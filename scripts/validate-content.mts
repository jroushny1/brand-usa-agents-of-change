...
// 3. Every webinar should be reachable from the homepage — via a card
// or the conference-talks section (isConferenceTalk entries).
for (const [id, w] of Object.entries(webinarData)) {
  if (!cardIds.has(id) && !w.isConferenceTalk) {
...
console.log(`ok: ${dataIds.size} webinars, ${cardIds.size} homepage cards — content is consistent`)
